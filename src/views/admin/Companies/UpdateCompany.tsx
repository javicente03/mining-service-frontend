...
import { GetCompanyById } from "../../../helpers/admin/companies";
...
export const UpdateCompany = () => {
    ...
    const company = useQuery(['GetCompanyById', id], () => GetCompanyById(id))

    useEffect(() => {
        if (company.data?.data) {
            setData({
                ...data,
                razon_social: company.data.data?.razon_social,
                telefono: company.data.data?.telefono,
                direccion: company.data.data?.direccion,
            })
            updateRut(company.data?.data.rut);
        }
    }, [company.data])
    ...
    const updateCompany = mutatorRequest('/admin/companies/update/'+id, 'PUT', {
        ...data,
        rut: rut.raw
    })
    ...
    useEffect(() => {
        ...
        if (updateCompany.isError) {
            // @ts-ignore
            setViewAlert({ ...viewAlert, open: true, message: updateCompany.error.response.data.error, color: 'error' });
        }
    }, [updateCompany.isSuccess, updateCompany.isError])
